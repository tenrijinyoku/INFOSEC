...
public class mysqlmanipulate {
    ...
    public static void AddItem(String database,String table,String serial,String alg,long startdate,long enddate,String publickey,String privatekey){
        LoadDriver();
        Connection conn;
        String url="jdbc:mysql://localhost:3300/"+database;
        try {
    ...
    public static void CreateTable(String port,String databases,String table,String user,String password){
        ...
        try {
            ...
            Statement stmt = conn.createStatement(); //创建Statement对象
            System.out.print("成功连接到数据库！");
            String sql =  "create table "+table+" (uid VARCHAR(100),serial VARCHAR(100) , alg VARCHAR(100),publickey TEXT, privatekey TEXT ,startdate BIGINT, enddate BIGINT, issuer VARCHAR(100),version VARCHAR(100)) ";
            // 执行SQL语句
            int resultSet = stmt.executeUpdate(sql);
    ...
    public static void CreateTable_User(String port,String databases,String table,String user,String password){
        LoadDriver();
        String url="jdbc:mysql://localhost:"+port+"/"+databases;    //JDBC的URL
        //调用DriverManager对象的getConnection()方法，获得一个Connection对象
        Connection conn;
        try {
            ...
            Statement stmt = conn.createStatement(); //创建Statement对象
            System.out.print("成功连接到数据库！");
            String sql =  "create table "+table+" (uid VARCHAR(100),username VARCHAR(100),password VARCHAR(100) ,createtime BIGINT) ";
            // 执行SQL语句
            int resultSet = stmt.executeUpdate(sql);
            System.out.println("新表创建成功！！");
            // 关闭连接和释放资源
    ...
    public static void main(String[]args){
        ...
        //CreateTable("3300","customer","Ca","root","25832233l");
        CreateTable_User("3300","customer","User","root","25832233l");
